		...
		<section className={`popup ${props.isOpen ? 'popup_opened' : undefined}`}>
			<div className="popup__container">
				...
				<form className="popup__form" name={props.name} onSubmit={props.onSubmit}>
					...
					{props.children}
					<span className="popup__email-alert">{props.errorMessage}</span>
					<button className={`popup__submit ${props.isValid ? 'popup__submit_active' : undefined}`} type="submit" disabled={props.isValid ? false : true}>{props.submitText}</button>
				</form>
